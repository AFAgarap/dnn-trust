...
class NeuralNet(tf.keras.Model):
    def __init__(self, **kwargs):
        ...
        self.hidden_layer_1 = tf.keras.layers.Dense(units=kwargs['units'][0], activation=tf.nn.relu)
        self.hidden_layer_2 = tf.keras.layers.Dense(units=kwargs['units'][1], activation=tf.nn.relu)
        self.dropout_layer = tf.keras.layers.Dropout(rate=kwargs['dropout_rate'])
        self.output_layer = tf.keras.layers.Dense(units=kwargs['num_classes'])
        self.optimizer = tf.optimizers.SGD(learning_rate=kwargs['learning_rate'], momentum=kwargs['momentum'])
